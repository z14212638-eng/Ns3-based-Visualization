...
int main(int argc, char *argv[]) {
  ...
  mac.SetEdca(AC_BE, "TxopLimits",
              StringValue(std::to_string(txopLimit) + "us"));
  ...
  allStaDevices.Add(staDeviceA);
  allStaDevices.Add(staDeviceB);
  allStaDevices.Add(staDeviceC);
  allStaDevices.Add(staDeviceD);

  NetDeviceContainer allApDevices;
  allApDevices.Add(apDeviceA);
  allApDevices.Add(apDeviceB);
  allApDevices.Add(apDeviceC);
  allApDevices.Add(apDeviceD);

  Ptr<SniffUtils> Sniff_Utils = CreateObject<SniffUtils>();
  Sniff_Utils->Initialize(allStaDevices, allApDevices, simulationTime.ToDouble(Time::S));
  ...
  if (txopLimit) {
    ...
    if (verifyResults && txopLimit &&
      ...
      NS_LOG_ERROR("Maximum TXOP duration "
                   << netA.m_max << " is not in the expected boundaries!");
      exit(1);
    }
  }

  throughput =
      totalPacketsThroughB * payloadSize * 8 / simulationTime.GetMicroSeconds();
  ...
  if (txopLimit) {
    ...
    if (verifyResults &&
        (netB.m_max < MicroSeconds(3350) || netB.m_max > MicroSeconds(3520))) {
      NS_LOG_ERROR("Maximum TXOP duration "
                   << netB.m_max << " is not in the expected boundaries!");
      exit(1);
    }
  }

  throughput =
      totalPacketsThroughC * payloadSize * 8 / simulationTime.GetMicroSeconds();
  ...
  if (txopLimit) {
    ...
    if (verifyResults &&
        (netC.m_max < MicroSeconds(3350) || netC.m_max > MicroSeconds(3520))) {
      NS_LOG_ERROR("Maximum TXOP duration "
                   << netC.m_max << " is not in the expected boundaries!");
      exit(1);
    }
  }

  throughput =
      totalPacketsThroughD * payloadSize * 8 / simulationTime.GetMicroSeconds();
  ...
  if (txopLimit) {
    ...
    if (verifyResults && txopLimit &&
      ...
      NS_LOG_ERROR("Maximum TXOP duration "
                   << netD.m_max << " is not in the expected boundaries!");
      exit(1);
    }
  }
