...
main(int argc, char* argv[])
{
    ...
    apNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(3.08935, 5.30757, 2.61873));

    // STA Positions
    mobility.Install(staNodes);
    staNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(5.84125, 5.18032, 4.41621));
    ...
    // Flow 1: AP_0 -> STA_1 (Bulk)
    {
        BulkSendHelper bulk("ns3::TcpSocketFactory", Address(InetSocketAddress(staAddrs[1], basePort + 0)));
    ...
    // Flow 2: STA_0 -> AP_1 (Bulk)
    {
        BulkSendHelper bulk("ns3::TcpSocketFactory", Address(InetSocketAddress(apIfaces.GetAddress(1), basePort + 1)));
        bulk.SetAttribute("SendSize", UintegerValue(512));
        bulk.SetAttribute("MaxBytes", UintegerValue(1000));
